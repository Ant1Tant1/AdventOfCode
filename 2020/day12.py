...
def get_action(i=0, facing_dir=0, dir_x=0, dir_y=0):
    ...
    if char == 'N':
        ...
        dir_y += round(sin(facing_dir*pi/180) * value, 1)
        dir_x += round(cos(facing_dir*pi/180) * value, 1)
...
def get_action_2(i=0, ship_pos=np.array([0,0]), dir=np.array([10,1])):
    ...
    if char == 'N':
        ...
        new_x = (dir[0] * cos(-value*pi/180) - dir[1] * sin(-value*pi/180))
        new_y = (dir[0] * sin(-value*pi/180) + dir[1] * cos(-value*pi/180))
